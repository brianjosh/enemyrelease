...
#Code to support "Aboveground release from enemies increases seedling survival in grasslands"
#Code prepared by:
...
#Part 1: Libraries and notes                --- Line 25
#Part 2: Early/mid site plots in context    --- Line 61
#Part 3: Damage analysis                    --- Line 99
#Part 4: Phylogenetic analysis              --- Line 204
#Part 5: Traits                             --- Line 364
#Part 6: Survival analysis                  --- Line 430
#Part 7: Main figures                       --- Line 897
#Part 8: Supplementary figures              --- Line 1025
#A joke to finish                           --- Line 1214
...
#Part 2: Early/mid site plots in context

#Checking that the two sets of plots (early and mid) display two distinct 
#clusters, and that they represent early and mid sites more generally

#Read in dataframe containing our plots where all planted seedlings died (i.e. they represent
#unplanted controls at our sites) and all early and mid plots at Cedar Creek.

#Data from Catford et al. (2023) Higher plant colonisation and lower resident diversity in 
#grasslands more recently abandoned from agriculture. Journal of Ecology, 111(11), 2424-2440

tax_df <- read.csv("siteNMDS.csv", row.names = 1) 

#Run the NMDS
nmds = metaMDS(tax_df[,4:110], try = 50, trymax = 100) ## will take while to run, increased the number of tries as solution marginally variable

#Extract first two axes
nmds$points %>% as.data.frame() %>% bind_cols(tax_df) %>%
  dplyr::select(MDS1, MDS2, group, exp) -> nmds_df

#Identify the MechER plots so they can be highlighted
nmds_df %>% dplyr::filter(exp == "MechER") -> nmds_mechER

nmds_df$group <- factor(nmds_df$group, levels = c("Early", "Mid"))
nmds_mechER$group <- factor(nmds_mechER$group, levels = c("Early", "Mid"))

#Redo NMDS just for our plots

tax_df %>% dplyr::filter(exp == "MechER") -> Mtax_df
Mnmds = metaMDS(Mtax_df[,4:110], try = 50, trymax = 100) ## will take while to run, increased the number of tries as solution marginally variable

Mnmds$points %>% as.data.frame() %>% bind_cols(Mtax_df) %>%
  dplyr::select(MDS1, MDS2, group, exp) -> Mnmds_df

Mnmds_df$group <- factor(Mnmds_df$group, levels = c("Early", "Mid"))

###########################################################################################

#Part 3: Damage analysis
...
damageM$plot <- as.factor(damageM$plot)
...
#We tested for the effect of richness because results found an unexpected
#positive relationship between plot-level richness and seedling survival
...
#Interaction between damage and functional group tested and found to
#be non-significant

insectmod <- lm(ef ~ meaninsectdamageC + growth, data=insecticide)
summary(insectmod)
plot(insectmod)
#one clear outlier: repeat without outlier
insecticideadj <- insecticide[-12, ]
insectmodadj <- lm(ef ~ meaninsectdamageC + growth, data=insecticideadj)
summary(insectmodadj)

fungalmod <- lm(ef ~ meaninsectfungaldamageC + growth, data=fungicide)
summary(fungalmod)
plot(fungalmod)
#again, one clear outlier: repeat without outlier
fungicideadj <- fungicide[-10, ]
fungalmodadj <- lm(ef ~ meaninsectfungaldamageC + growth, data=fungicideadj)
summary(fungalmodadj)

fungalmod <- lm(ef ~ meanfungaldamageC + growth, data=fungicide)
summary(fungalmod)
#without outlier
fungicideadj <- fungicide[-10, ]
...
sp_file <- effectsize%>%
  dplyr::select(species, genus, family, species.relative, genus.relative)%>%
  distinct() #remove the duplicate names
...
trait_file <- effectsize%>%
  dplyr::select(specieslowbar, effectsizeI, effectsizeIF)
...
#Now repeat while removing the two outliers (identified 
#in the damage assessments) (P. virgatum and M. fistulosa)
...
#Run PCAs
traits.pca <- prcomp(traitspace2[ ,c(2:6)], scale. = T)
summary(traits.pca)
pcaresult <- traits.pca$x
write.csv(pcaresult, file="abovegroundpcaresults.csv")

#Modified version of 'pcaresult' was then combined with
#effect sizes to produce 'pca_results_modified.csv', which we will now read in

pcaplotting <- read.csv("pca_results_modified.csv", header=T, stringsAsFactors = T)

#See if there is a relationship between aboveground traits and effect sizes, and
#whether this varies with functional group
#Interactions were tested and were non-significant

#Effect of I vs. PC1aboveground
mod1 <- lm(logeffectsizeI ~ PC1aboveground + funcgroup, data=pcaplotting)
Anova(mod1, type="III")
summary(mod1)

#Effect of I vs. PC2aboveground
mod2 <- lm(logeffectsizeI ~ PC2aboveground + funcgroup, data=pcaplotting)
Anova(mod2, type="III")
summary(mod2)

#Effect of IF vs. PC1aboveground
mod3 <- lm(logeffectsizeIF ~ PC1aboveground + funcgroup, data=pcaplotting)
Anova(mod3, type="III")
summary(mod3)

#Effect of IF vs. PC2aboveground
mod4 <- lm(logeffectsizeIF ~ PC2aboveground + funcgroup, data=pcaplotting)
Anova(mod4, type="III")
summary(mod4)

#See if there is a relationship for SLA or leaf N separately 
mod5 <- lm(logeffectsizeI ~ SLA + funcgroup, data=pcaplotting)
Anova(mod5, type="III")
summary(mod5)

mod6 <- lm(logeffectsizeI ~ N + funcgroup, data=pcaplotting)
Anova(mod6, type="III")
summary(mod6)

mod7 <- lm(logeffectsizeIF ~ SLA + funcgroup, data=pcaplotting)
Anova(mod7, type="III")
summary(mod7)

mod8 <- lm(logeffectsizeIF ~ N + funcgroup, data=pcaplotting)
Anova(mod8, type="III")
summary(mod8)

###########################################################################################

#Part 6: Survival analysis 
...
survival <- survival %>% 
  dplyr::mutate(across(c(awMPD, lightavailable, moisture, heightinitial), scale)) %>%
  dplyr::mutate(across(c(awMPD, lightavailable, moisture, heightinitial), as.numeric))
...
propdataall2 <- dplyr::rename(propdataall2, richness = ...12)
...
#Model Set 1a: Analyse time period by time period
#(Interactions tested and non-significant)

Ju22prop2 <- glmmTMB(mean ~ treatment + functional + awMPD +
                            stage + lightavailable + moisture + richness
                          + (1|functional/species),
                          family=gaussian,
                          data=June22ndprop2)
summary(Ju22prop2)
...
confint(Ju22prop2)
simulationOutput <- simulateResiduals(fittedModel = Ju22prop2, plot=F)
plot(simulationOutput)

Ju30prop2 <- glmmTMB(mean ~ treatment + functional + awMPD +
                       stage + lightavailable + moisture + richness
                     + (1|functional/species),
                     family=gaussian, 
                     data=June30thprop2)
...
confint(Ju30prop2)
simulationOutput <- simulateResiduals(fittedModel = Ju30prop2, plot=F)
plot(simulationOutput)

Jl6prop2 <- glmmTMB(mean ~ treatment + functional + awMPD +
                       stage + lightavailable + moisture + richness
                     + (1|functional/species),
                     family=gaussian, 
                     data=July6thprop2)
...
confint(Jl6prop2)
simulationOutput <- simulateResiduals(fittedModel = Jl6prop2, plot=F)
plot(simulationOutput)

Jl18prop2 <- glmmTMB(mean ~ treatment + functional + awMPD +
                      stage + lightavailable + moisture + richness
                    + (1|functional/species),
                    family=gaussian, 
                    data=July18thprop2)
...
confint(Jl18prop2)
simulationOutput <- simulateResiduals(fittedModel = Jl18prop2, plot=F)
plot(simulationOutput)

Jl25prop2 <- glmmTMB(mean ~ treatment + functional + awMPD +
                       stage + lightavailable + moisture + richness
                     + (1|functional/species),
                     family=gaussian, 
                     data=July25thprop2)
...
confint(Jl25prop2)
simulationOutput <- simulateResiduals(fittedModel = Jl25prop2, plot=F)
plot(simulationOutput)

Jl31prop2 <- glmmTMB(mean ~ treatment + functional + awMPD +
                       stage + lightavailable + moisture + richness
                     + (1|functional/species),
                     family=gaussian, 
                     data=July31stprop2)
...
confint(Jl31prop2)
simulationOutput <- simulateResiduals(fittedModel = Jl31prop2, plot=F)
plot(simulationOutput)

Au8prop2 <- glmmTMB(mean ~ treatment + functional + awMPD +
                       stage + lightavailable + moisture + richness
                     + (1|functional/species),
                     family=gaussian, 
                     data=August8thprop2)
...
confint(Au8prop2)
simulationOutput <- simulateResiduals(fittedModel = Au8prop2, plot=F)
plot(simulationOutput)
...
#Interaction between functional group and treatment tested, no interaction

dead <- read.csv("new_dead.csv", header=T, stringsAsFactors = T)

newdeadmodel <- glmmTMB(newdead ~ treatment*date + functional + awMPD +
                      stage + lightavailable + moisture + richness +
                        (1|functional/species),
                    family=poisson, 
                    data=dead)
...
simulationOutput <- simulateResiduals(fittedModel = newdeadmodel, plot=F)
plot(simulationOutput)
...
#Check no difference between the heights of different treatments
height <- lm(heightinitial ~ treatment, data=June22nd)
Anova(height)

#June 22nd
fullsurvivalmodelJn22 <- glmmTMB(survived ~ treatment + stage + heightinitial +
                                  functional + (1|functional/species/plotnumber), 
...
#August 8th
fullsurvivalmodelAu8 <- glmmTMB(survived ~ treatment + stage + heightinitial + 
                                  functional + (1|functional/species/plotnumber), 
                                family=binomial(link="logit"), 
                                data=August8th)
...
fig1a <- ggplot(data=propdataallplotting, aes(x=date, y=mean, color=treatment)) +
  geom_smooth(aes(fill=treatment, linetype=stage), alpha=0.1) +
  scale_colour_manual(values=c('#003cffff', '#999999','#E69F00')) +
  scale_fill_manual(values=c('#003cffff', '#999999','#E69F00')) +
...
#read in file containing effect sizes
...
fig1b <- ggplot(data=effect, aes(x=time, y=ef, group=treatment, color=treatment)) +
  geom_point(position = position_dodge(0.5), size=2.5) + 
  geom_errorbar(aes(ymin=cilower, ymax=ciupper), width=0.4,
                position=position_dodge(0.5)) +
  theme_classic() +
  scale_color_manual(values=c('#999999','#E69F00')) +
  xlab("Weeks after transplanting") + 
  ylab("Proportional change in seedling \nsurvival relative to control") +
  labs(color="Treatment") +
  geom_hline(yintercept=0, linetype="dashed", color='#003cffff') +
  ylim(-0.05, 0.2) + theme(legend.position = c(0.22, 0.82))
fig1b
...
fig2 <- ggplot() +
  ...
  xlab("Plot richness") + labs(color="Treatment")+
...
fig3 <- ggplot(data=speffect, aes(x=species, y=ef, group=treatment, color=treatment)) +
  ...
  geom_hline(yintercept=1, linetype="dashed", color='#003cffff') +
...
fig4 <- barplot.phylo4d(phylofile)

#Figure 5

#Figure 5a

fig5a <- ggbiplot(traits.pca, labels=NULL, scale=0) +
  geom_point(size=2) + 
...
fig5a
ggsave("pca.svg", width=100, height=100, units="mm")

#Figure 5b
fig5b <- ggplot(pcaplotting, aes(x=PC1aboveground, y=logeffectsizeI)) +
  geom_point(size=2, color='#999999') + geom_smooth(method=lm, color='#999999', fill='#999999') +
  xlab("PC1 Leaf Traits (49.0% explained var.)") + ylab("Insecticide effect size (log)") +
...
figS1 <- ggplot(pcaplotting, aes(x=PC1aboveground, y=PC2aboveground, 
                                          color=funcgroup)) +
...
figS3a <- ggplot() +
  theme_bw() +
  geom_point(aes(x=MDS1, y=MDS2, shape=group, fill=group), alpha = 1, shape=21, size = 4, data=Mnmds_df) +
  scale_colour_manual(values=c("Early" = "#88CCEE", "Mid" = "#CC79A7")) +
  scale_fill_manual(values=c("Early" = "#88CCEE", "Mid" = "#CC79A7")) +
...
figS3b <- ggplot() +
  geom_hdr(aes(x=MDS1, y=MDS2, fill=group), data=nmds_df, show.legend = FALSE) +
  geom_point(aes(x=MDS1, y=MDS2, fill=group), alpha = 0.2, data=nmds_df, shape= 21) +
  theme_bw() +
  guides(colour=guide_legend("Succession stage"), fill = "none") +
  geom_point(aes(x=MDS1, y=MDS2, shape=group, fill=group), alpha = 1, shape=21, size = 4, data=nmds_mechER) +
  scale_colour_manual(values=c("Early" = "#88CCEE", "Mid" = "#CC79A7")) +
  scale_fill_manual(values=c("Early" = "#88CCEE", "Mid" = "#CC79A7")) +
  ylab("NMDS2") + xlab("NMDS1") +
  xlim(-3, 2) + ylim(-2, 2) +
...
figS3 <- ggarrange(figS3a, figS3b, ncol=2, labels=c("A", "B"))
figS3

ggsave(figS3, filename="NMDS.svg", width = 160, height = 100, units="mm")
...
figS4a <- ggplot(data=insectdamage, aes(x=Stage, y=mean, fill=Treatment)) +
...
figS4b <- ggplot(data=fungaldamage, aes(x=Stage, y=mean, fill=Treatment)) +
  geom_bar(stat="identity", position=position_dodge()) +
  ylab("Mean percent of leaf area damaged (±SE)") + xlab("Community successional stage") +
  scale_fill_brewer(palette="Blues") + theme_classic() +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1,
...
figS4 <- ggarrange(figS4a, figS4b, nrow=2, labels=c("A", "B"))
figS4
...
figS5 <- ggplot(data=effectind, aes(x=time, y=ef, group=treatment, color=treatment)) +
  geom_point(position = position_dodge(0.5), size=2.5) + 
  geom_errorbar(aes(ymin=cilower, ymax=ciupper), width=0.5,
                position=position_dodge(0.5)) +
  theme_classic() +
  scale_color_manual(values=c('#999999','#E69F00')) +
  xlab("Weeks after planting") + ylab("Odds of seedling survival relative to control") + 
  labs(color="Treatment") +
  geom_hline(yintercept=1, linetype="dashed", color='#003cffff') +
  ylim(0.5, 3)
figS5
...
figS6 <- ggplot(data=dead, aes(x=date, y=newdead, color=treatment)) +
...
#Figure S7
#Plot the effect sizes of stage and richness through time
...
stage <- ggplot(data=effects, aes(x=time, y=efstage)) +
  ...
  ylab("Proportion surviving in mid-succesion \nrelative to early-succesion community") +
  geom_hline(yintercept=0, linetype="dashed") +
  theme(axis.title.x = element_blank()) +
  ylim(-0.2, 0.2) + theme(axis.title = element_text(size=9))
stage
...
richness <- ggplot(data=effects, aes(x=time, y=efrichness)) +
  ...
  theme_classic() +
  xlab("Weeks after transplanting") + 
  ylab("Relative proportion surviving for \neach additional species in the plot") +
  geom_hline(yintercept=0, linetype="dashed") +
  ylim(-0.2, 0.2) + theme(axis.title = element_text(size=9))
richness

figS7 <- ggarrange(stage, richness, nrow=2, 
                             labels=c("A", "B"))
figS7

#Figure S8

figS8 <- ggplot(data=August8thprop2, aes(x=species, y=mean, 
...
figS9a <- ggplot(insecticide, aes(x=meaninsectdamageC, y=ef, color=growth)) +
...
figS9b <- ggplot(fungicide, aes(x=meaninsectfungaldamageC, y=ef, color=growth)) +
  geom_point(size=2.5) + scale_colour_manual(values=c('#999999','#E69F00')) + 
...
figS9 <- ggarrange(figS9a, figS9b, nrow=2, labels=c("A", "B"))
figS9
