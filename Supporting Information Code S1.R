...
#Code to support "Aboveground enemy release increases seedling survival in grasslands"
#Code prepared by:
...
#Part 2: Damage analysis                 --- Line 63
#Part 3: Phylogenetic analysis           --- Line 166
#Part 4: Traits                          --- Line 327
#Part 5: Survival analysis               --- Line 422
#Part 6: Effect sizes and naturalisation --- Line 863
#Part 7: Main figures                    --- Line 930
#Part 8: Supplementary figures           --- Line 1145
#A joke to finish                        --- Line 1366
...
#Part 6: Linking effect sizes with naturalisation

regions <- read.csv("regions.csv", header=T, stringsAsFactors = T)
str(regions)

#Remove the outlier A. gerardii (identified using Cook's distance in initial exploration)
regions2 <- subset(regions, species != "A. gerardi")

forbs <- subset(regions, form=="forb")
grasses <- subset(regions, form=="grass")
forbs2 <- subset(regions2, form=="forb")
grasses2 <- subset(regions2, form=="grass")

#Statistical analyses - first with outlier removed

#Overall I vs. regions
mod1 <- lm(logregions ~ logef_I, data=regions2)
summary(mod1)

#Overall IF vs. regions
mod2 <- lm(logregions ~ logef_IF, data=regions2)
summary(mod2)

#Forbs I vs. regions
mod3 <- lm(logregions ~ logef_I, data=forbs2)
summary(mod3)

#Forbs IF vs. regions
mod4 <- lm(logregions ~ logef_IF, data=forbs2)
summary(mod4)

#Grasses I vs. regions
mod5 <- lm(logregions ~ logef_I, data=grasses2)
summary(mod5)

#Grasses IF vs. regions
mod6 <- lm(logregions ~ logef_IF, data=grasses2)
summary(mod6)

#Repeat with outlier included

#Overall I vs. regions
mod7 <- lm(logregions ~ logef_I, data=regions)
summary(mod7)

#Overall IF vs. regions
mod8 <- lm(logregions ~ logef_IF, data=regions)
summary(mod8)

#Forbs I vs. regions
mod9 <- lm(logregions ~ logef_I, data=forbs)
summary(mod9)

#Forbs IF vs. regions
mod10 <- lm(logregions ~ logef_IF, data=forbs)
summary(mod10)

#Grasses I vs. regions
mod11 <- lm(logregions ~ logef_I, data=grasses)
summary(mod11)

#Grasses IF vs. regions
mod12 <- lm(logregions ~ logef_IF, data=grasses)
summary(mod12)
...
#Figure 5a
fig5a <- ggbiplot(traits.pca, groups=traitspace2$funcgroup, labels=NULL, 
                   scale=0) +
  geom_point(aes(color=traitspace2$funcgroup), size=2) + 
  scale_colour_manual(values=c('#AA4499','#009E73')) + 
...
fig5b <- ggplot(pcaplotting, aes(x=PC1aboveground, y=logeffectsizeI, 
  ...
  scale_colour_manual(values=c('#AA4499','#009E73')) + 
  scale_fill_manual(values=c('#AA4499','#009E73')) +
...
#Figure 6a
fig6a <- ggplot(regions2, aes(x=logef_I, y=logregions, color=form)) +
  geom_point() + geom_smooth(method="lm", color="black") +
  scale_colour_manual(values=c('#AA4499','#009E73')) +
  theme_classic() + xlab("Insectide effect size (log)") +
  ylab("Non-native naturalised regions (log)") +
  theme(legend.position='none')
fig6a

#Figure 6b
fig6b <- ggplot(regions2, aes(x=logef_IF, y=logregions, color=form)) +
  geom_point() + geom_smooth(method="lm", color="black", linetype="dashed") +
  scale_colour_manual(values=c('#AA4499','#009E73')) +
  theme_classic() + xlab("Insectide+fungicide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank()) +
  theme(legend.position='none')
fig6b

#Figure 6c
fig6c <- ggplot(forbs2, aes(x=logef_I, y=logregions)) +
  geom_point(color='#AA4499') + geom_smooth(method="lm", color='#AA4499', fill='#AA4499') +
  theme_classic() + xlab("Insectide effect size (log)") +
  ylab("Non-native naturalised regions (log)") 
fig6c

#Figure 6e
fig6e <- ggplot(forbs2, aes(x=logef_IF, y=logregions)) +
  geom_point(color='#AA4499') + geom_smooth(method="lm", linetype="dashed",
                                            color='#AA4499', fill='#AA4499') +
  theme_classic() + xlab("Insectide+fungicide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank())
fig6e

#Figure 6d
fig6d <- ggplot(grasses2, aes(x=logef_I, y=logregions)) +
  geom_point(color='#009E73') + geom_smooth(method="lm", color='#009E73', 
                                            fill='#009E73', linetype="dashed") +
  theme_classic() + xlab("Insectide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank())
fig6d

#Figure 6f
fig6f <- ggplot(grasses2, aes(x=logef_IF, y=logregions)) +
  geom_point(color='#009E73') + geom_smooth(method="lm", color='#009E73', 
                                            fill='#009E73', linetype="dashed") +
  theme_classic() + xlab("Insectide+fungicide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank())
...
overallplot <- ggarrange(fig6a, fig6b, ncol=2)
subplots <- ggarrange(fig6c, fig6d, fig6e, fig6f, ncol=4)
fig6 <- ggarrange(overallplot, subplots, nrow=2)
fig6
...
figS7 <- ggarrange(plot4, plot5, plot6, nrow=3, 
                             labels=c("A", "B", "C"))
figS7
...
figS9 <- ggplot(data=August8thprop2, aes(x=species, y=mean, 
...
#Figure S10a
figS10a <- ggplot(regions, aes(x=logef_I, y=logregions)) +
  geom_point() + geom_smooth(method="lm", color="black") +
  theme_classic() + xlab("Insectide effect size (log)") +
  ylab("Non-native naturalised regions (log)")
figS10a
...
#Figure S10b
figS10b <- ggplot(regions, aes(x=logef_IF, y=logregions)) +
  geom_point() + geom_smooth(method="lm", color="black", linetype="dashed") +
  theme_classic() + xlab("Insectide+fungicide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank())
figS10b
...
#Figure S10c
figS10c <- ggplot(forbs, aes(x=logef_I, y=logregions)) +
  geom_point() + geom_smooth(method="lm", color="black") +
  theme_classic() + xlab("Insectide effect size (log)") +
  ylab("Non-native naturalised regions (log)") 
figS10c
...
#Figure S10e
figS10e <- ggplot(forbs, aes(x=logef_IF, y=logregions)) +
  geom_point() + geom_smooth(method="lm", color="black", linetype="dashed") +
  theme_classic() + xlab("Insectide+fungicide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank())
figS10e
...
#Figure S10d
figS10d <- ggplot(grasses, aes(x=logef_I, y=logregions)) +
  geom_point() + geom_smooth(method="lm", color="black", linetype="dashed") +
  theme_classic() + xlab("Insectide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank())
figS10d
...
#Figure S10f
figS10f <- ggplot(grasses, aes(x=logef_IF, y=logregions)) +
  geom_point() + geom_smooth(method="lm", color="black", linetype="dashed") +
  theme_classic() + xlab("Insectide+fungicide effect size (log)") +
  ylab("Non-native naturalised regions (log)") + theme(axis.title.y=element_blank())
figS10f

overallplot2 <- ggarrange(figS10a, figS10b, ncol=2)
subplots2 <- ggarrange(figS10c, figS10d, figS10e, figS10f, ncol=4)
figS10 <- ggarrange(overallplot2, subplots2, nrow=2)
figS10
